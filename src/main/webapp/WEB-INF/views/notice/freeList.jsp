            	...
            	<tr>
	                ...
	                <form action="noticeList.do">
	                 	...
	                 	<select name="col">
	                        ...
	                        <input id="word" type="text" name="word" size="35px" placeholder="검색어를 입력해주세요." value="${dto}">
		...
		<c:if test="${requestScope.count>0 }">
			...
			<div class="">
				...
				<c:if test="${startPage>0}">
					<a href="./freeList.do?pageNum=${startPage}">[이전]</a>
				</c:if>

				<c:forEach var="i" begin="${startPage+1}" end="${endPage-1}">
					<a href="./freeList.do?pageNum=${i}">[${i}]</a>
				</c:forEach>

				<c:if test="${endPage<pageCount}">
					<a href="./freeList.do?pageNum=${startPage+11}">[다음]</a>
				</c:if>
			</div>
		</c:if>
